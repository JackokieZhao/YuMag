...
%		\thanks{Q. Yu is with the School of Electronic Science and Engineering, Nanjing University, No.163, Xianlin Road, Qixia District, Nanjing, Jiangsu Province, China, 200240, (e-mail: yuquan61@qq.com)}
%		\thanks{Q. Yu,  J. Zhao, and H. Zhou are with the School of Electronic Science and Engineering, Nanjing University, No.163, Xianlin Road, Qixia District, Nanjing, Jiangsu Province, China, 200240, (e-mail: yuquan61@qq.com, haibozhouuw@gmail.com, jackokie@gmail.com)}
%		\thanks{X. Shen is with the Department of Electrical and
%			Computer Engineering, University of Waterloo, 200 University Avenue West,
%			Waterloo, Ontario, Canada, N2L 3G1, (e-mail:sshen@uwaterloo.ca)}
...
%				\begin{figure*}[htbp]
%				\centering
%				\subfigure[Electrical synaptic transmission.]{
%					\begin{minipage}[t]{0.45\linewidth}
%						\centering
%						\includegraphics[width=0.8\linewidth]{figures/electronic_trans.pdf}
%						\label{fig: electronic_trans}
%				\end{minipage}}
%				\subfigure[Chemical synaptic transmission.]{
%					\begin{minipage}[t]{0.48\linewidth}
%						\centering
%						\includegraphics[width=0.8\linewidth]{figures/chemical_trans.pdf}	
%						\label{fig: chemical_trans}
%				\end{minipage}}
%				\par
%				\subfigure[The parallel characteristic of information reception,
%				fusion and processing.]{
%					\begin{minipage}[t]{0.48\linewidth}
%						\includegraphics[width=\linewidth]{figures/parallel.pdf}
%						\label{fig: neuron_parallel}
%				\end{minipage}}
...
%			\end{figure*}
			\subsubsection{Information Transmission between Neurons}
				...
				\begin{figure*}[htbp]
					...
					\subfigure[Electrical synaptic transmission.]{
						\begin{minipage}[t]{0.4\linewidth}
							\centering
							\includegraphics[width=0.9\linewidth]{figures/electronic_trans.pdf}
							\label{fig: electronic_trans}
					\end{minipage}}
					\subfigure[Chemical synaptic transmission.]{
				...
				Fig. \ref{fig: chemical_trans} shows the transmission mechanism of chemical synaptic.		
				The presynaptic and postsynaptic membranes at chemical synapses are separated by a synaptic cleft.
				The presynaptic is usually composed by axon terminal. 	
				...
				Synaptic integration constitutes the neural computation for transformation of many synaptic inputs to a single neuronal output, 
				by which multiple synaptic potentials combine within one postsynaptic neuron.
...
%				The fact that synaptic strength and transmitter release can be modified provides physical basis for memory, learning and imagination. 
%				For example, in Fig. \ref{fig: cooperation} there are four cases for long-term potentiation (LTP) in the hippocampus. As the first instance shows, single and weak signal from axon is not sufficient to open the ion channel gate, nor lead to LTP. In case 2 and 3, when some inputs arrive at the dendrites cooperatively and simultaneously, action potential is triggered and results in LTP in all axon links. Similarly, the strong inputs generate LTP in related axon links. Without stimulation, there is no LTP even when neighboring synapses receive strong signals.In summary, the selectivity modifies the synapses with memories through plastic adaptation toward diverse stimulations, resulting in memory, learning and other functional developments in brain.
%				\begin{figure}[htbp]
%					\centering
%					\includegraphics[width=3.5in]{figures/cooperation.pdf}
%					\caption{The short-range distributed cooperation in the hippocampus.}	
...
%				\begin{figure}[htbp]
%					\centering
%					\includegraphics[width=3in]{figures/neuron_scale}
%					\caption{Human neuron network is a scale-free network.}	
%					\label{fig: neuron_scale}
%				\end{figure}
			...
			\subsubsection{Hierarchical and Heterogeneous Network Architecture}			
				...
				In addition, neurons in the cerebral cortex are organized in layers, including molecular layer, external granule cell layer, external pyramidal cell layer, internal granule cell layer, internal pyramidal cell layer and multiform layer. And each of these layers obtains different functions.
...
%			Fig. \ref{fig: node_analogy} shows the  analog of neron and NANET node we proposed.
%			\begin{figure}[htbp]
%				\centering
%				\includegraphics[width=3.6in, height=2.0in]{figures/node_analogy.pdf}
%				\caption{The Structure Analog between HWN Node and Neuron}
%				\label{fig: node_analogy}
%			\end{figure}
...
%			\begin{figure}[htbp]
%				\centering
%				\includegraphics[width=3.2in, height=1.6in]{figures/network_analogy}
%				\caption{AdHoc Network Demo}	
%				\label{fig: analog_net_topology}
%			\end{figure}
%			
	...
	\label{section: general_design}
		...
		\subsection{Design of NANET Node}
			...
			\subsubsection{Design of Information Transmission and Reception Mechanism}
				In the design of NANET nodes, information is sent employing omnidirectional antennas and relayed in the form of broadcast. 
				Therefore, the information transmission is unidirectional in NANET node.
				Information reception of HWN node utilize multiple directional antennas to separate different channels and receive information through orientation. 
				Multi-channel antennas' application could contribute to the reducing interference as well as facilitating the fusion of multi-channel information.
				...
				And the multichannel reception would effectively promote the spectrum efficiency, transmitting speed, and service quality in HWN.
				Since the transmission of information is in a broadcast mode, the node also assists other nodes in the transmitting information when forwarding its own received information, 
				which achieving the effect of cooperative transmission between the nodes.
			...
			\subsubsection{Design of Information Fushion and Processing}
				...
				When the fusion of multi-channel information received by directional antenna satisfy certain conditions, for example, the threshold, 
				the NANET node will foward or store the fused information.
				Information is encoded in the frequency of electromagnetic wave.
				Thus, the information of directional antenna is converted into frequency coding, which owns relatively strong anti-interference ability.
		...
		\subsection{Design of General NANET Architecture}
			Fig. \ref{fig: sys_frame} represents the design of general NANET architecture.
			\begin{figure*}[htbp]
				\centering
				\includegraphics[width=0.8\linewidth]{figures/sys_frame.pdf}
				\caption{Design of general NANET architecture, which contains three layers: space, air, and groud.}	
				\label{fig: sys_frame}
			\end{figure*}
			\subsubsection{Scale-Free Network}
			...
			\subsubsection{Seperation of Service and Control Channels}
				In the information transmission of NANET, the traffic channel is separated from the control channel.
				Generally, the control channel perform as a synchronization mechanism for information transmission of multiple nodes, 
				and information transmission in control channel is bi-directional, fast, and reliable when compared to service channel.                  
				...
				Afterwards, characteristic of reliability and unchangeability can be further guaranteed at the protocol level.
				At the same time, we can use the wired network or reduce the number of hops (such as satellite communications) in the control channel information transmission, 
				and ensure the speed and the efficiency of the control channel transmission from the physical layer.
				...
				From the aspect of network's transmission efficiency, 
				the ground node of NANET might require many hops to reach the target node for most nodes while the satellite node can almost connect to any node by one hop.
				So the transmission efficiency of satellite communication is the highest, 
				and its delay may also be affected by the number of hops which is much lower compared to the ground HWN.
				Of course, its resistance to damage and robustness are the worst, too.
				Limited by the bandwidth, satellite communications can be used as control channels(wired channels can be used as control channel for its transmission speed), and conventional AdHoc networks are used as Service channels, which realizes the characteristic that control channel is dramatically faster than the Service channels.
			...
			\subsubsection{Distributed and Centerless Network Framework}
				...
				And the network's distribution is reflected in the distribution of network framework and the distribution of information transmission.
				The NANET is a self-growing network, which determines that the network structure is unlikely to be a centralized network.
				Owing to fllfilling various requirements of service in different areas, the network would develop into a distributed network in network framework.
				...
				and the NANET always possess the backbone from the perpective of entire system architecture.
...
%	\begin{IEEEbiography} [{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./figures/authors/yuquan.jpg}}]{Quan Yu}
%		received the B.S. degree in information physics from Nanjing University in 1986, the M.S. degree in radio wave propagation from Xidian University in 1988, and the Ph.D. degree in fiber optics from the University of Limoges in 1992. Since 1992, he joined the faculty of the Institute of China Electronic System Engineering Corporation, where he has been a Senior Engineer, and currently a Research Fellow. His main areas of research interest are the architecture of wireless networks, optimization of protocols and cognitive radios. Dr. Yu is a Fellow of Chinese Academy of Engineering (CAE).
%	\end{IEEEbiography}
%
%	\begin{IEEEbiography} [{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./figures/authors/jiwei.jpg}}]{Jackokie(Jiwei) Zhao}
%		received the M.S. degree in information and communication system from Xidian University in 2018.
%		He is currenctly a Prepare-Doctor with YuLab in Institute of Space-Terrestrial Intelligent Networks(ISTIN) Group, Nanjing University. 
%		He wined the first prize in the 2016 CCF(China Computer Federation) China Big Data & Computing Intelligence Contest.
%		His current research interests  mainly focus on resource management and protocol design in cognitive radio network and vehicular networks.
%	\end{IEEEbiography}
%
%	\begin{IEEEbiography} [{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./figures/authors/haibo.jpg}}]{Haibo Zhou}
%		received the Ph.D. degree in Information and Communication Engineering from Shanghai Jiaotong University, Shanghai, China, in 2014. He is currently a Post-Doctoral Fellow with the Broadband Communications Research (BBCR) Group, University of Waterloo. His current research interests include resource management and protocol design in cognitive radio networks and vehicular networks.
%	\end{IEEEbiography}
%
%	\begin{IEEEbiography} [{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./figures/authors/sherman.jpg}}]{Xuemin (Sherman) Shen [M'97-SM'02-F'09]}
%		is a University Professor, Department
%		of Electrical and Computer Engineering, University of
%		Waterloo, Canada. He is also the Associate Chair for Graduate
%		Studies. Dr. Shen's research focuses
%		on resource management in interconnected wireless/wired
%		networks, wireless network security, social networks, smart
%		grid, and vehicular ad hoc and sensor networks. Dr. Shen served as
%		the Technical Program Committee Chair/Co-Chair for IEEE
%		Globecom'16, Infocom'14, IEEE VTC'10 Fall, the Symposia Chair for IEEE
%		ICC'10, the Tutorial Chair for IEEE VTC'11 Spring and
%		IEEE ICC'08, the Technical Program Committee Chair for
%		IEEE Globecom'07. He also serves/served as the
%		Editor-in-Chief for IEEE Network, IEEE Internet of Things
%		Journal (IoT-J), Peer-to-Peer Networking and Application,
%		and IET Communications; a Founding Area Editor for IEEE
%		Transactions on Wireless Communications; an Associate
%		Editor for IEEE Transactions on Vehicular Technology,
%		Computer Networks, and ACM/Wireless Networks, etc.
%		Dr. Shen is a registered Professional Engineer of Ontario,
%		Canada, an IEEE Fellow, a Royal Society of Canada Fellow, an Engineering Institute of Canada
%		Fellow, a Canadian Academy of Engineering Fellow, and a
